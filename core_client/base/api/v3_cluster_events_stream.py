"""Streaming endpoint: ``POST /api/v3/cluster/events``.

Endless SSE stream of cluster log events. Registered on ``AsyncClient`` as an
...
def _build_request(client: Client, filters=None):
    return "POST", f"{client.base_url}/api/v3/cluster/events", serialize_filters(filters)
