"""Streaming endpoint: ``POST /api/v3/cluster/events/process``.

Endless NDJSON stream of process events (e.g. ``type="progress"``). Registered on
``AsyncClient`` as an async-generator method (see ``_add_stream_method``).
"""
...
def _build_request(client: Client, filters=None):
    return "POST", f"{client.base_url}/api/v3/cluster/events/process", serialize_filters(filters)
