...
from ..models.v3 import ClusterNodeState
...
def _build_request(
    ...
    return {
        "method": "get",
        "url": f"{client.base_url}/api/v3/cluster/node/{id}/state",
...
def _build_response(response: httpx.Response):
    if response.status_code == 200:
        response_200 = TypeAdapter(ClusterNodeState).validate_python(response.json())
        return response_200
    else:
